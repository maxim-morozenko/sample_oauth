...
class AdminOrderController extends Controller
{
    public function index(Request $request)
    {
        try {
            ...
            $cAdminUser = Auth::id();
            $aAllowedUsers = [1, 55, 65];
            if ( !in_array($cAdminUser, $aAllowedUsers) ) {
                if ($lData->list) {
                    foreach ($lData->list as $keyOrder => $oOrder) {


                        $lData->list[$keyOrder]->legal_inn = substr_replace($oOrder->legal_inn, '****', 0, 4);
                        $lData->list[$keyOrder]->legal_ogrn = substr_replace($oOrder->legal_ogrn, '****', 0, 4);
                        $lData->list[$keyOrder]->legal_full_name = substr_replace($oOrder->legal_full_name, '****', 0, 4);

                        if ($oOrder->address) {
                            $sStrReplace = '';
                            for ($i = 0; $i <= (int)strlen($oOrder->address) / 2; $i++) {
                                $sStrReplace .= '*';
                            }
                            $lData->list[$keyOrder]->address = substr_replace($oOrder->address, $sStrReplace, 0, ((int)strlen($oOrder->address) / 2) - 1);

                        }


                        $lData->list[$keyOrder]->contact_phone = substr_replace($oOrder->contact_phone, '********', 0, 8);
                        $lData->list[$keyOrder]->contact_email = substr_replace($oOrder->contact_email, '****', 0, 4);
                        $lData->list[$keyOrder]->contact_name = substr_replace($oOrder->contact_name, '****', 0, 4);
    ...
    public function getLastCarRequest(Request $request)
    {
        try {
            ...
            if ( !in_array($cAdminUser, $aAllowedUsers) ) {
                        ...
                        $orderCar->legal_inn = substr_replace($orderCar->legal_inn, '****', 0, 4);
                        $orderCar->legal_ogrn = substr_replace($orderCar->legal_ogrn, '****', 0, 4);
                        $orderCar->legal_full_name = substr_replace($orderCar->legal_full_name, '****', 0, 4);

                        if ($orderCar->address) {
                            $sStrReplace = '';
                            for ($i = 0; $i <= (int)strlen($orderCar->address) / 2; $i++) {
                                $sStrReplace .= '*';
                            }
                            $orderCar->address = substr_replace($orderCar->address, $sStrReplace, 0, ((int)strlen($orderCar->address) / 2) - 1);

                        }

                $orderCar->contact_phone = substr_replace($orderCar->contact_phone, '****', 0, 4);
                $orderCar->contact_email = substr_replace($orderCar->contact_email, '****', 0, 4);
                $orderCar->contact_name = substr_replace($orderCar->contact_name, '****', 0, 4);
            ...
            if ( $order ) {
                ...
                if ( $order->user ) {
                    ...
                    $cOrderUser->notify((new CantCallNotification($cOrderUser, $order))
                        ...
                        ->delay(Carbon::now()->addSeconds(0.2)));
                }
            }
    ...
    public function listForCalendar(Request $request)
    {
        ...
        try {
            ...
            $filter['statuses'] = $request->filter['statuses'];
            ...
            $lDrivers = User::getByParams([
                'roles' => User::USER_DRIVER_ROLES
            ]);
            ...
            if ( $lCar->count ) {
                foreach ($lCar->list as $keyIndex => $oItem) {
                    ...
                    $oDriver = new User();
                    if ($lDrivers) {
                        foreach ($lDrivers->list as $keyDriver => $iDriver) {
                            if ( $iDriver->id == $oItem->delivery_id ) {
                                $oDriver = $iDriver;
                            }
                        }
                    }
                    ...
                    $oItem->title = __('calendar.car_title', [
                        ...
                        'driver' => $oDriver->name . ' ' . $oDriver->last_name
                    ]);
                    $oItem->color = [
                        'primary' => '#ffa726',
